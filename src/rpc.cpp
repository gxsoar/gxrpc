...
int rpcc::bind(TO to) {
  int r;
  int ret = call(rpc_const::bind, r, to, 0);
  if (ret == 0) {
...
void rpcc::cancel(void) {
  ...
  for (auto &[first, second] : calls_) {
    caller *ca = second;

    jsl_log(JSL_DBG_2, "rpcc::cancel: force caller to fail\n");
    {
      std::unique_lock<std::mutex> cl(ca->m);
      ca->done = true;
...
int rpcc::call1(unsigned int proc, marshall &req, unmarshall &rep, TO to) {
  ...
  while (true) {
    ...
    {
      std::unique_lock<std::mutex> cal(ca.m);
      // 使用条件变量如果caller没有发送就一直陷入这个循环直到超时
      while (!ca.done) {
        jsl_log(JSL_DBG_2, "rpcc:call1: wait\n");
        auto time_out = std::chrono::system_clock::now() + std::chrono::seconds(nextdeadline.tv_sec);
        if (ca.c.wait_until(ca.m, time_out) == ETIMEDOUT) {
  ...
  std::scoped_lock<std::mutex> cal(ca.m);
...
void rpcc::get_refconn(connection **ch) {
  std::scoped_lock<std::mutex> ml(chan_m_);
...
bool rpcc::got_pdu(connection *c, char *b, int sz) {
  ...
  std::scoped_lock<std::mutex> cl(ca->m);
  if (!ca->done) {
    ca->un->take_in(rep);
    ca->intret = h.ret;
    if (ca->intret < 0) {
...
rpcs::rpcs(unsigned int p1, int count)
  ...
  dispatchpool_ = new ThreadPool(6, false);
...
void rpcs::reg1(unsigned int proc, handler *h) {
  std::unique_lock<std::mutex> pl(procs_m_);
...
void rpcs::updatestat(unsigned int proc) {
  std::scoped_lock<std::mutex> cl(count_m_);
  counts_[proc]++;
  curr_counts_--;
  if (curr_counts_ == 0) {
    ...
    std::scoped_lock<std::mutex> rwl(reply_window_m_);
...
void rpcs::dispatch(djob_t *j) {
  ...
  if (h.clt_nonce) {
    ...
    {
      std::unique_lock<std::mutex> rwl(reply_window_m_);
    ...
    {
      std::unique_lock<std::mutex> rwl(conss_m_);
...
void marshall::rawbyte(unsigned char x) {
  if (ind_ >= capa_) {
    capa_ *= 2;  //  如果读写头的位置大于容量的位置，执行2倍扩容
    VERIFY(buf_ != NULL);
    buf_.reset(new char(capa_));
  }
  buf_.get()[ind_++] = x;
}
...
void marshall::rawbytes(const char *p, int n) {
  if ((ind_ + n) > capa_) {
    capa_ = capa_ > n ? 2 * capa_ : (capa_ + n);
    VERIFY(buf_.get() != nullptr);
    buf_.reset(new char(capa_));
  }
  memcpy(buf_.get() + ind_, p, n);
  ind_ += n;
}
...
void unmarshall::rawbytes(std::string &ss, unsigned int n) {
  if ((ind_ + n) > (unsigned)sz_) {
    ok_ = false;
  } else {
    std::string tmps = std::string(buf_.get() + ind_, n);
    swap(ss, tmps);
    VERIFY(ss.size() == n);
    ind_ += n;
  }
